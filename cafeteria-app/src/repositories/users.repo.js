...
const insert = db.prepare(`
  ...
  VALUES (@email, @passwordHash, @role, @name, @phone, @createdAt)
`);
const updateProfile = db.prepare('UPDATE users SET name=@name, phone=@phone WHERE id=@id');
const countAdmins = db.prepare("SELECT COUNT(*) AS n FROM users WHERE role = 'admin'");

function toPublic(row) {
  if (!row) return null;
  return { id: row.id, email: row.email, role: row.role, name: row.name, phone: row.phone };
}
...
export const usersRepo = {
  ...
  create({ email, passwordHash, role = 'customer', name = '', phone = '' }) {
  ...
  updateProfile(id, { name, phone }) {
    updateProfile.run({ id: Number(id), name: name || '', phone: phone || '' });
